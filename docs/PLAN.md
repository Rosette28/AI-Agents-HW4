...
### ADR-006: Multi-bug generalization as an extension (Task F), not core scope

- **Status:** Accepted
- **Context:** BugsInPy's HTTPie project contains 5 bugs total. Making all 5
  full first-class deliverables (each with its own diagrams, vault pages,
  agent run, fix, before/after screenshots, and token comparison) would
  roughly 5x the Phase 2-3 workload, which is too large for two people and a
  limited free-tier LLM account (NFR4).
- **Decision:** Bug #3 remains the single primary deep-dive for Tasks A-E
  (graph, vault, architecture diagrams, agent run, fix, before/after, token
  comparison). As an additional original extension (Task F), the
  graph-guided agent built for Bug #3 is re-run against all 5 BugsInPy
  HTTPie bugs (using each bug's own buggy commit/test), and the results
  (root_cause_found, tokens_used, llm_calls, files_read, iterations per bug)
  are summarized in a single small report/table.
- **Alternatives considered:** Treat all 5 bugs as equal core scope -
  rejected per the workload/quota reasoning above.
- **Trade-offs:** Demonstrates generalization of the graph-guided approach
  across multiple bugs without requiring per-bug diagrams, vaults, or
  screenshots; only one extra report file and one extra agent-run loop are
  needed.
...
### 6.5 `reports/multi_bug_summary.md` (Task F extension contract)

Must contain a Markdown table with one row per BugsInPy HTTPie bug (1-5),
columns: `bug_id | bug_summary | root_cause_found | tokens_used | llm_calls |
files_read | iterations`, generated by re-running the graph-guided agent
(`graphify_agent agent --mode graph_guided --bug <id>`) against each bug's
buggy commit/test, plus a short paragraph noting how well the approach
generalizes beyond Bug #3.
